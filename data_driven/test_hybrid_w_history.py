...
def compute_nextpos_and_scores_hybrid(row, model):
    ...
    prev_lat, prev_lon = row['Latitude_prev'], row['Longitude_prev']
    prev_position = torch.unsqueeze(torch.tensor([prev_lat, prev_lon], dtype = torch.float), axis=0)
    ...
    pos_pred = model(xphys,prev_position, final_context)
...
if __name__ == "__main__": 
    ...
    module = HybridDriftModule_w_History.load_from_checkpoint(checkpoint_path)
    model = module.model
    model.eval()
