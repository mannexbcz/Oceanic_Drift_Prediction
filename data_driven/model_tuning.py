...
csvfile_train= '/data/manon/MasterThesis/NOAA/nextpoint_ds/contexts/pt32d50/next_point_dataset_train_ok.csv'
csvfile_val= '/data/manon/MasterThesis/NOAA/nextpoint_ds/contexts/pt32d50/next_point_dataset_val_ok.csv'
chkpt_folder = '~/checkpoints/MasterThesis/tuning2'
bs = 32
num_workers= 32
min_epochs = 150
max_epochs = 1000
...
def objective(trial: optuna.trial.Trial):
    ...
    # Parameters
    channel1 = trial.suggest_int("channel1", 16,512)
    channel2 = trial.suggest_int("channel2", 16,512)
    hidden1 = trial.suggest_int("hidden1", 16,512)
    hidden2 = trial.suggest_int("hidden2", 16,512)
    hidden3 = trial.suggest_int("hidden3", 16,512)
    hidden4 = trial.suggest_int("hidden4", 16,512)
    ...
    trainer = pl.Trainer(default_root_dir=chkpt_folder,min_epochs=min_epochs, max_epochs=max_epochs,check_val_every_n_epoch=check_val_every_n_epochs,enable_progress_bar=False,enable_model_summary = False, callbacks=[EarlyStopping(monitor="val_loss", patience = patience, mode="min"), lr_monitor, checkpoint_callback, checkpoint_callback2],accelerator="gpu", devices=1) #profiler="simple",
